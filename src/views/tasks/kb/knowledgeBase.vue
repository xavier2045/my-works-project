...
<script lang='ts' setup>
  ...
  const findRouterKbDetailsFun = (item: any) => {
    const currentUrl = window.location.href; // 获取当前页面的 URL
    let daoId = extractIdFromUrl(currentUrl)+'';
    // router.push({
    //   name: 'kbDetails',
    //   query: {
    //     id: 1
    //   }
    // })
    userInfo.knowledgeBaseData = item;
    setItem('knowledgeBaseData', item);
    router.push({ name: 'KbDetails', params: { id: daoId} })
  }
