        ...
        <Transition name="fade-slide" mode="out-in">
          <div class="work_main_top" v-if="currentStep === 1" key="step1">
            ...
            <div class="work_main_top_right">
              ...
              <div class="item-main">
                <div class="item-main-two">
                  ...
                  <div class="item-main-two-r inputBg">
                    ...
                    <div class="item-main-two-r-m relievoShadow" :class="{ focused: allowReCreateEr }">
                      <div class="item-main-flex-head">
                        <div class="head-main">
                          <div class="item-main-icon">
                          ...
                          <div class="item-main-name">
                            ...
                            <i 
                              ...
                              @click.stop="toggleDropdown('modele3')"></i>
                          </div>
                        </div>
                      </div>
                      <div class="item-main-content">
                        <div class="item-main-time-group-input">
                          <div class="item-main-time-group-input-num">
                            <SwitchBut v-model="isallowReCreate" :labelText="['是', '否']" /> 
                          </div>
                        </div>
                      </div>
                    </div>
                    <div class="item-main-pulldown-module dropdown" v-if="currentDropdown === 'modele3'" @click.stop>
                      <div class="module-content">
                        “二创”表示您允许其他用户对您的作品进行免费二次创作；同时，若您上传了作品源件，其他用户也可下载该源件，请知悉。
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="item-main">
                <div class="item-main-source relievoShadow" :class="{ focused: statementEr || citeWorkIdsEr }">
              ...
              <div class="item-main">
                <div class="item-main-two">
                  <div class="item-main-two-l inputBg">
                    ...
                    <div class="item-main-two-l-m relievoShadow" :class="{ focused: isOnChainEr }">
                      <div class="item-main-flex-head">
                        <div class="head-main">
                          <div class="item-main-icon">
                            <i class="icon font_family icon-xinren24"></i>
                          ...
                          <div class="item-main-name">
                            ...
                            <i 
                              class="icon font_family icon-wenhaoyuanquan20 cursorPointer"
                              @click.stop="toggleDropdown('modele4')"></i>
                          </div>
                        </div>
                      </div>
                      <div class="item-main-content">
                        <div class="item-main-time-group-input">
                          <div class="item-main-time-group-input-num">
                            <SwitchBut v-model="isOnChain" :labelText="['是', '否']"  /> 
                          </div>
                          <!-- <div class="item-main-time-group-input-unit"></div> -->
                        </div>
                      </div>
                    ...
                    <div class="item-main-pulldown-module dropdown" v-if="currentDropdown === 'modele4'" @click.stop>
                      <div class="module-content">
                        去中心化存储指的是将您的作品永久保存于区块链上，从而确保您对数字作品的所有权。
                      </div>
                    </div>
                  </div>
                  <div class="item-main-two-r inputBg">
          ...
          >
            ...
            <div class="submitTheRelease">
              ...
              <div class="check-clause">
                ...
                <div class="check-clause-text">发布即代表您已阅读并同意<span @click="findTypeMsgPopupFun(1)">《用户发布作品声明告知条款》</span></div>
              </div>
            </div>
          </div>
        </Transition>
...
<script lang='ts' setup>
  ...
  const allowReCreateEr = ref(false);
  ...
  const isOnChain = ref(true);
  const isallowReCreate = ref(false);
  const reqData = ref({
  ...
  const convertContents = ref([]); // 任务介绍内容转换
  ...
  const getTaskDraft = () => {
    ...
    return new Promise<void>((resolve, reject) => {
      worksDraftData(resData).then((response: any) => {
        ...
        isallowReCreate.value = resultData.allowReCreate === 1 ? true : false;
  ...
  // 添加共同创作者
  const addCoCreator = () => {
    // 直接使用 selectedItems.value 来更新 coCreatorList 和 reqData.coCreators
    coCreatorList.value = [...selectedItems.value];
    reqData.value.coCreators = selectedItems.value.map(item => item.userId);

    console.log('coCreatorList.value', coCreatorList.value);
    ...
    const target = event.target as HTMLInputElement;
      if (target.files) {
        ...
        allowedFiles.forEach((file) => {
          ...
          console.log('worksuploadStore', worksuploadStore);
        });
  ...
  const getData = () => {
    ...
    reqData.value.allowReCreate = isallowReCreate.value ? 1 : 0;
    reqData.value.isOnChain = isOnChain.value ? 1 : 0;
    reqData.value.patentUrls = [];
  ...
  const publish = async () => {
      ...
      try {
        ...
        for (const files of worksuploadStore.attsFiles) {
          ...
          console.log('附件', files.isprevious);
  ...
  const verifyForm = (item: any) => {
    ...
    if (item == 'originalType') {
      if(reqData.value.originalType !== '') {
        originalTypeEr.value = false;
      }
    }
    
    if (item == 'allowReCreate') {
      if(reqData.value.allowReCreate !== '') {
        allowReCreateEr.value = false;
      }
    }
    if (item == 'description') {
    ...
    if (item == 'statement') {
      if(reqData.value.statement !== '') {
        descriptionEr.value = false;
      }
    }
    if (item == 'citeWorkIds') {
      if(reqData.value.citeWorkIds !== '') {
        citeWorkIdsEr.value = false;
      }
    }
    isUnll.value = true;
  ...
  onMounted(async() => {
    ...
    if(userInfo.openworkType == 1) {
      if(getItem('work') !== null) {
        console.log('获取缓存的resData', getItem('work'));
        ...
        if(originalType !== null) {
          originalTypeName.value = originalType === 1 ? '原创' : '非原创';
        }
        if(allowReCreate !== null) {
          isallowReCreate.value = allowReCreate === 1 ? true : false;
        }
        if(reqData.value.isOnChain !== null) {
          isOnChain.value = reqData.value.isOnChain === 1 ? true : false;
        }
        if(publicType !== null) {
          publicTypeName.value = publicType === 1 ? '所有人可见' : 
                        publicType === 2 ? '仅互关可见' : 
                        '仅自己可见';
        }
        ...
        //   setTimeout(() => resolve(reqData.value.tags), 1000)
        // );
        // reqData.value.tags = [...response];
