<template>
        ...
        <div class="node-header">
          <div class="node-header-flex">
            <div class="dow-select" v-if="currentFilterCriteria.businessType !== '3'">
                <TimeSelect
                    :options="selectList"
                    v-model="currentFilterCriteria.businessId"
                    placeholder="所有提案"
                    @open="handleOpen" 
                    @close="handleClose"
            ...
            <div class="dow-select">
                <TimeSelect
                    :options="typeList"
                    v-model="currentFilterCriteria.isPass"
                    placeholder="采纳状态"
                    @open="handleOpen" 
                    @close="handleClose"
            ...
            <div class="node-content-main-bg">
                <div class="table-header">
                    <ul>
                        ...
                        <li>投票</li>
                        <li>采纳</li>
                        <li v-if="proType == 1 || proType == 3">标记</li>
                    </ul>
                </div>
                    ...
                    >
                        <template #item="{ items }">
                          <div v-if="items.length === 0" class="list-item-tips">暂无数据</div>
                            ...
                            :key="refreshKey" 
                            class="list-item"
                            @click="findRouterProposDetailsFun(item)"
                            >
                            <div class="list-item-main">
                                ...
                                <div class="cell column2">
                                    <div class="cell-main">
                                      <div class="cell-main-name">{{ item.createTime }}</div>
                                    </div>
                                </div>
                                ...
                                <div class="cell column4">
                                    <div class="cell-main">
                                      <div class="cell-main-progress">
                                        <div class="agreeNum">
                                          <div class="agreeNum-item" v-for="(items, index) in item.agreeUserAvatars" :key="index">
                                            <img :src="items" alt="" />
                                          </div>
                                        </div>
                                        <div class="disAgreeNum">
                                          <div class="disAgreeNum-item" v-for="(items, index) in item.disagreeUserAvatars" :key="index">
                                            <img :src="items" alt="" />
                                          </div>
                                        </div>
                                        <!-- <div class="cell-main-progress-bar"
                                         :style="{ width: Math.max((Number(item.agreeNum) / (Number(item.agreeNum) + Number(item.disAgreeNum))) * 100, 7) + '%' }"
                                         ></div> -->
                                         <div class="cell-main-progress-bar"
                                          :style="{ width: Math.max((Number(item.agreeNum) / (Number(item.agreeNum) + Number(item.disAgreeNum))) * 100, 7) + '%' }"
                                          ></div>
                                ...
                                <div class="cell column4">
                                    <div class="cell-main">
                                      <div class="cell-main-status">
                                        ...
                                        <template v-else-if="item.isPass === 0">
                                          <i class="icon font_family icon-shenhebutongguo20 type-color3"></i>
                                        </template>
                                        <template v-else-if="item.isPass === null">
                                ...
                                <div class="cell column5" v-if="proType == 1 || proType == 3">
                                    <div class="cell-main">
                                      <div class="cell-main-name">{{ item.markName }}</div>
                                    </div>
                                </div>
                                </div>
  ...
  <script lang='ts' setup>
    ...
    import { proposalsList, tasksListNodeData, nodeDataListCalendar } from '@/api/dao';
    ...
    const updateTypeValue = (val: string) => {
      currentFilterCriteria.value.isPass = val;
    };
    ...
    const fetchNews = async (page: number) => {
      let getData = {
          pageNum: page,
          pageSize: 16,
          isPass: currentFilterCriteria.value.isPass,
      ...
      try {
        ...
        if(data.rows.length == 0) {
          ...
          userInfo.proposalIdList = data.rows.map((item: { proposalId: any; }) => item.proposalId);
          return data.rows;
        }
      } catch (error) {
              ...
              .list-item-main{
                ...
                .cell{
                  ...
                  .cell-main-progress{
                    ...
                    top: 50%;
                    border-radius: 5px;
                    background: linear-gradient(90deg, #dcedf7 0%, #00b3ff 100%);
                    .agreeNum{
                      position: absolute;
                      top: -23px;
                      left: 0;
                    ...
                    .cell-main-progress-bar{
                      height: 10px;
                      position: absolute;
                      top: 0;
                      left: 0;
                      border-radius: 5px;
                      background: linear-gradient(90deg, #743511 0%, #eb3737 100%);
                    }
                  }
                  .cell-main-status{
                    width: 100%;
